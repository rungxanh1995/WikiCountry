...
class CountryDetailDataSource: NSObject {
	...
	internal func getCountry() -> Country { return country }
...
extension CountryDetailDataSource: UITableViewDataSource {
	...
	func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
		...
		case .languages, .langCodes:	return country.languages.count
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		case .flag:
			...
			if let cell = cell as? FlagCell { cell.configure(for: country) }
			return cell
		case .general:
			let cell = tableView.dequeueReusableCell(withIdentifier: Constants.infoCellId, for: indexPath)
...
extension CountryDetailDataSource {
	final func buildName() -> String {
		if country.name == country.nativeName { return "Name: \(country.name)" }
		return "Name: \(country.name) · \(country.nativeName)"
	}
	
	final func buildRegion() -> String {
		if country.region == .empty { return "Region: N/A" }
		return "Region: \(country.region.rawValue)"
	}
	
	final func buildDemonym() -> String {
		if country.demonym == "" { return "Demonym: N/A" }
		return "Demonym: \(country.demonym)"
	}
	
	final func buildCapital() -> String {
		if country.capital == "" { return "Capital: N/A" }
		return "Capital: \(country.capital)"
	}
	
	final func buildPopulation() -> String {
		if let population = Utils.numberFormatter.string(for: country.population) { return "Population: \(population)" }
		return "Population: N/A"
	}
	
	final func buildArea() -> String {
		if let area = Utils.numberFormatter.string(for: country.area) { return "Area: \(area) km²" }
		return "Area: N/A"
	}
	
	final func buildTimezone(_ timezone: String) -> String {
		return "\(timezone)"
	}
	
	final func buildLangCode(_ language: Language) -> String {
		if let iso6391 = language.iso6391 { return "\(iso6391), \(language.iso6392)" }
		return "\(language.iso6392)"
	}
	
	final func buildLanguage(_ language: Language) -> String {
		if language.name == language.nativeName { return "\(language.name)" }
		return "\(language.name) · \(language.nativeName)"
	}
	
	final func buildCurrency(_ currency: Currency) -> String {
	...
	final func createShareText() -> String {
		var text = """
		About \(country.name)
		General
		-\t\(buildName())
